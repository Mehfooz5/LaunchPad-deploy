...
import { FaUsers, FaBriefcase, FaRocket, FaChartLine, FaHandshake, FaArrowRight } from "react-icons/fa"
...
const GradientButton = ({ children, href, primary = true, className = "" }) => {
  return (
    <motion.a
      href={href}
      whileHover={{ scale: 1.05, boxShadow: "0 10px 25px -5px rgba(59, 130, 246, 0.5)" }}
      whileTap={{ scale: 0.95 }}
      className={`relative group overflow-hidden px-8 py-4 rounded-full text-lg font-bold transition-all duration-300 inline-flex items-center gap-2 ${
        primary ? "bg-gradient-to-r from-blue-600 to-purple-600 text-white" : "bg-white text-blue-900"
      } ${className}`}
    >
      <span className="relative z-10">{children}</span>
      <motion.span
        className={`absolute inset-0 ${
          primary ? "bg-gradient-to-r from-blue-500 to-purple-500" : "bg-gradient-to-r from-gray-100 to-white"
        } opacity-0 group-hover:opacity-100 transition-opacity duration-300`}
      />
      <motion.span className="absolute right-8 opacity-0 group-hover:opacity-100 group-hover:right-6 transition-all duration-300">
        <FaArrowRight />
      </motion.span>
    </motion.a>
  )
...
export default function HomePage() {
  ...
  const heroImageY = useTransform(scrollYProgress, [0, 0.2], [0, -30])
  ...
  const testimonials = [
    {
      ...
      text: "The platform connected us with investors who not only provided capital but also strategic guidance that helped us scale our operations across three continents in just 18 months.",
    },
    {
      name: "Michael Chen",
      role: "Angel Investor",
      image: "https://randomuser.me/api/portraits/men/54.jpg",
      text: "I've discovered some of my most successful investments through this platform. The quality of startups and the detailed insights provided make my due diligence process much more efficient.",
    },
    {
      name: "Aisha Patel",
      role: "Founder, HealthSync",
      image: "https://randomuser.me/api/portraits/women/45.jpg",
      text: "From concept to Series A in 14 months - that's the power of the right connections. The mentorship we received was invaluable in refining our business model.",
    },
  ]
